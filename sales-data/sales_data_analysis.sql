...
drop table if exists practice.rfm_summary;
create table practice.rfm_summary 
with rfm as 
(
	select 
		CUSTOMERNAME, 
...
-- select  
-- 		case 
-- 			when rfm_cell >= 10 then "High Level Customer"
-- 			when rfm_cell < 6 then "Low Level Customer"
-- 			else "Mid Level Customer"
-- 		end as rfm_level,
--         round(avg(Recency),2) as mean_recency ,
--         round(avg(Frequency),2) as mean_frequency,
--         round(avg(MonetaryValue),2) as mean_monetary,
